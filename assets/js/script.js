...
function displayTrendCoins(coins) {
    ...
    coins.forEach(coin => {
        ...
        `;
        row.onclick = () => window.location.href = `../../crypto-marketplace/pages/coin.html?coin=${coinData.id}`;
...
function displayAssets(data) {
    ...
    data.forEach(asset => {
        ...
        row.onclick = () => window.location.href = `../../crypto-marketplace/pages/coin.html?coin=${asset.id}`;
    });
